...
def learn_skill(
    ...
    summary = agent_client.skill_index.get(skill_id.strip())
    if summary is None:
    ...
    absolute_file, resolve_error, _ = _resolve_under_root(skill_root, relative_segment)
        ...
        try:
            lines = sorted(
                (
                    ...
                    if not _is_internal_skill_path(str(path.relative_to(skill_root)))
                ),
                key=str.casefold,
            )
        except OSError as exception:
...
async def run_skill_script(
    ...
    """
    absolute_script, error = _resolve_script_for_run(
        agent_client,
        skill_id,
        relative_path,
    )
    ...
    completed = await _invoke_script(command)
    if isinstance(completed, str):
        return completed

    result = _format_process_output(completed)
    if len(result) <= 10_000:
        return result
    return _persist_oversized_output(result)


def _resolve_script_for_run(
    agent_client: typing.Any,
    skill_id: str,
    ...
    if not skill_id or not skill_id.strip():
        ...
        summary = agent_client.skill_index.get(trimmed_skill_id)
        if summary is None:
            ...
            elif _is_internal_skill_path(relative_segment):
                error = (
                    "错误：下划线开头的路径为技能内部文件，"
                    "不可通过 run_skill_script 执行。"
                )
            else:
                script, resolve_error, file_missing = _resolve_under_root(
                    skill_root,
                    relative_segment,
                    allow_missing=True,
                )
                if resolve_error is None:
                    ...
                    error = "\n".join(
                        [
                            ...
                            "",
                            learn_skill(agent_client, trimmed_skill_id),
                        ]
                    )
                else:
                    error = resolve_error
...
def _persist_oversized_output(result: str) -> str:
    guid = str(uuid.uuid4())
...
def _is_internal_skill_path(relative_path: str) -> bool:
