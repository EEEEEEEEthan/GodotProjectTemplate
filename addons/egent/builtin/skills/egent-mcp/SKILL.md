...
```text
Agent (mcp_bridge)  ←stdio→  egent.mcp.server  ←HTTP→  GodotMcp (游戏进程)
                              game_command 工具         POST /mcp
```

| 层级 | 位置 | 职责 |
|------|------|------|
| Agent 桥接 | `addons/egent/builtin/agent/mcp_bridge.py` | 启动 stdio MCP 服务、发现 `game_command` 工具 |
| MCP 转发 | `addons/egent/mcp/server.py` | 将 `game_command` 转为 HTTP 请求 |
| 游戏 HTTP | `addons/egent/mcp/game_mcp.gd` | 监听端口、解析 POST `/mcp`、回调 handler |
...
`launch_game` 工具轮询启动日志获取端口；也可手动在 `addons/egent/.temp/game_*.log` 中查找。

## 验证清单

### 1. Agent 桥接（无需游戏）

使用 `egent_test_tool_run_egent_test` 工具。
...
- `egent_test_tool_run_egent_test` — 运行全部 egent 单元测试
